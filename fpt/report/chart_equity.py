"""Gráficos de evolução da banca — tendência OLS + ciclo (resíduo)."""
...
def decompose_equity_time(dates: np.ndarray, y: np.ndarray) -> dict:
    """
    Decomposição ciclo-tendência simples:
    tendência = regressão linear OLS no tempo; ciclo = série - tendência.
    """
...
def chart_equity_triple(
    ...
    """
    curve: DatetimeIndex → % retorno sobre banca inicial (0 = início).
    3 painéis: evolução | tendência OLS | ciclo (resíduo).
    """
    ...
    if "coef" in d:
        slope = d["coef"][0]
        ax.text(0.02, 0.97, f"Inclinação: {slope:+.3f} p.p./trade", transform=ax.transAxes, fontsize=8, va="top")
    ...
    _style_axes(ax, "Ciclo (série − tendência)", "Desvio (p.p.)", "Data")
    _format_date_axis(ax)
    ax.legend(loc="upper left", framealpha=0.95)
